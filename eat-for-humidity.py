...
if __name__ == "__main__":
    ...
    def get_pos(idx):
        ...
        y = index1 % 3
        return (x, y)
    def get_base_consumption(weight, T_e=25):
        """
        Weight is in kg,
    ...
    def get_fly_energy(weight, distance):
        ...
        return E_f
    def get_fire_energy(weight, x, y, T_e=25):
    ...
    def eat_when_age(age, T_e):
        ...
        ENERGY_GOT = 0
        base_cons = get_base_consumption(weight, T_e)
        ...
        while eaten < number_of_animals:
            ...
            temp1 = get_fire_energy(weight, x, y, T_e)
            fire_cos += temp1
        ...
        print(eaten_each)
        print('################################################################')
        return (ENERGY_CONSUMPTION, eaten_each)
    ...
    # Total energy for T_e = [25, 35]
    age = 280
    T_e = 25
    humidity = 'high'
    (energy_temp, eaten_temp) = eat_when_age(age, T_e)
    energy_n = np.array([energy_temp])
    eaten = np.array([eaten_temp[0]], dtype=int)
    # for T_e in range(a+1, b+1):
    #     (energy_temp, eaten_temp) = eat_when_age(age, T_e)
    #     energy_n = np.vstack([energy_n, energy_temp])
    #     eaten = np.vstack([eaten, eaten_temp[0]])
    np.savetxt(f'./data/energy-for-humidity={humidity}.txt', energy_n)
    np.savetxt(f'./data/eaten-for-humidity={humidity}.txt', eaten)

    # np.savetxt(f'./data/eaten-{years}-for.txt', eaten)