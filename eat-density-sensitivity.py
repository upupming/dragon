...
if __name__ == "__main__":
    ...
    def eat_when_age(age, density_scale, xi=0.1):
        global DENSITY
        DENSITY = np.array(DENSITY) * density_scale
    ...
    ########## Find and eat ############
    percentage_max = 0.08
    scales = np.linspace(1 * (1-percentage_max), 1 * (1+percentage_max), 9)
    # print(scales)
    # exit()
    # # print(xis)
    # print(xis[1:])
    # exit()
    AGE = 280
    eaten = np.array([eat_when_age(AGE, scales[0])[0]], dtype=int)
    
    for scale in scales[1:]:
        eaten = np.vstack([eaten, eat_when_age(AGE, scale)[0]])
        print(eaten)
    # plt.close()
